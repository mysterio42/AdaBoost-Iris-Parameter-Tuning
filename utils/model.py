...
def train_model(features, labels, args):
    ...
    if args.gs:
        ...
        gs = GridSearchCV(estimator=AdaBoostClassifier(), param_grid=params_grid, cv=10)
        ...
        plot_graph(gs.best_estimator_.estimators_[0], features, labels, 'adaboost-graph-gs-')
        ans = input('Do you want to save the model weight? ')
        if ans in ('yes', '1'):
            dump_model(gs.best_estimator_.estimators_[0], 'AdaBoost-gs-0')
            dump_model(gs.best_estimator_.estimators_[1], 'AdaBoost-gs-1')
            dump_model(gs.best_estimator_.estimators_[2], 'AdaBoost-gs-2')
        ...
        plot_graph(model.estimators_[0], features, labels, 'adaboost-graph-')
        ans = input('Do you want to save the model weight? ')
        if ans in ('yes', '1'):
            dump_model(model.estimators_[0], 'AdaBoost-')
